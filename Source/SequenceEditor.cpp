...
void SequenceEditor::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    if (buttonThatWasClicked == receiveButton)
    {
        ...
        uint8 data[5] = { 0b00000001, 0b00101111, 0b00000101, 0b00000011 , 0b01110111};
        MidiMessage m = MidiMessage::createSysExMessage(data, 5);
        deviceManager->getDefaultMidiOutput()->sendMessageNow(m);

        Logger::writeToLog("requested program from " + deviceManager->getDefaultMidiOutput()->getName());
...
void SequenceEditor::sendConfig() {
    ...
    for (int x = 0; x < 64;x++) {
        for (int y = 0;y < 6;y++) {
            if (grid[x][y].isEnabled()) {
                Logger::writeToLog("Sending config.");

                MidiBuffer buffer = MidiRPNGenerator::generate(1, nrpn + y * 128, grid[x][y].getNote(), true, true);
                deviceManager->getDefaultMidiOutput()->sendBlockOfMessagesNow(buffer);
                buffer = MidiRPNGenerator::generate(1, nrpn_v + y * 128, grid[x][y].getVelocity()+128, true, true);
                deviceManager->getDefaultMidiOutput()->sendBlockOfMessagesNow(buffer);
            }
            else {
                MidiBuffer buffer = MidiRPNGenerator::generate(1, nrpn + y * 128, 128, true, true);
                deviceManager->getDefaultMidiOutput()->sendBlockOfMessagesNow(buffer);
                buffer = MidiRPNGenerator::generate(1, nrpn_v + y * 128, 0, true, false);
                deviceManager->getDefaultMidiOutput()->sendBlockOfMessagesNow(buffer);
            }
