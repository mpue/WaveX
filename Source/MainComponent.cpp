...
MainContentComponent::MainContentComponent(TimeLine* timeLine, TrackPropertyView* trackProperties, MixerPanel* mixer) : thread("main")
{
    this->zoom = 21.0f;
...
void MainContentComponent::setupIO() {
...
void MainContentComponent::setupIO(int inchannels, int outchannels) {
    juce::BigInteger activeInputChannels = deviceManager.getCurrentAudioDevice()->getActiveInputChannels();
    juce::BigInteger activeOutputChannels = deviceManager.getCurrentAudioDevice()->getActiveOutputChannels();
...
void MainContentComponent::handleRecording(int _numSamples, const float** inputChannelData, float** outputChannelData) {
    for (int i = 0; i < navigator->getTracks().size();i++) {
...
double MainContentComponent::getRms(int channel) {
    if (channel == 0)
        return this->rmsLeft;
    else
...
double MainContentComponent::getMagnitude(int channel) {
    if (channel == 0)
...
bool MainContentComponent::isMasterVisible() {
    return this->masterVisible;
}

void MainContentComponent::setVolume(int channel, float volume) {
    if (channel == 0) {
        leftVolume = volume;
    }
    else {
...
void MainContentComponent::setPosition(long sample) {
    this->numSamples = sample;
}

TrackNavigator* MainContentComponent::getNavigator() {
    return this->navigator;
}

PositionMarker* MainContentComponent::getMarker() {
    return this->marker;
}

int MainContentComponent::getNumActiveChannels(int i) {
    i = i - ((i >> 1) & 0x55555555);
    i = (i & 0x33333333) + ((i >> 2) & 0x33333333);
    return (((i + (i >> 4)) & 0x0F0F0F0F) * 0x01010101) >> 24;
}

void MainContentComponent::importAudio() {
...
void MainContentComponent::removeSelectedTrack() {
    navigator->removeSelectedTrack();
}

void MainContentComponent::addTrack(Track::Type type) {
...
void MainContentComponent::openStepSequencer() {
...
void MainContentComponent::openEventRecorder() {
...
void MainContentComponent::setTracklength(long length) {
    this->tracklength = length;
}

long MainContentComponent::getTrackLength( ) {
    return tracklength;
}
...
PopupMenu MainContentComponent::getMenuForIndex(int index, const String & menuName) {
    PopupMenu menu;
